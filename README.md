...
- **Agent 检索智能体** — LLM 自主多步检索 + 反思，对比/多跳/歧义问题智能分解搜索，异常自动降级标准管线
- **文档管理** — 上传/解析/分块，支持 PDF、Word、Excel、PPT、TXT、Markdown
- **混合检索** — 向量语义检索 + BM25 关键词检索，RRF 融合排序
- **LLM 查询改写** — 短查询自动扩展、口语自动规范化，提升检索命中率
...
- **企业 IM 集成** — 企微/钉钉/飞书 Bot，员工在 IM 中直接问答，支持 Stream 长连接模式
...
| 对象存储 | MinIO | 原始文档存储 |
| 文档解析 | Qwen-VL + POI + Tika | PDF 视觉解析 + Office 原生解析 + 通用格式兜底 |
...
│   │   ├── evaluation/          # RAG 评测（数据集 + LLM-as-Judge 评分）
│   │   ├── permission/          # 文档级权限
...
| IM 回调 | `/api/v1/im/**` | 企微 Bot 回调端点 |
| IM 管理 | `/api/v1/admin/im/**` | 部门映射 + 身份关联管理（需 ADMIN） |
| 评测管理 | `/api/v1/admin/evaluation/**` | 数据集/问题管理 + 评测运行 + LLM-as-Judge 评分 |
...
用户输入 → FAQ 精确匹配
              ├─ 命中 → 短路返回预设答案（零延迟）
              └─ 未命中 → LLM 智能改写 → Glossary 规则改写
                            ├─ Agent 模式（可选）：改写结果作为提示 → LLM 多步检索 → 反思 → 合成
                            └─ 标准模式：改写结果直接搜索 → 混合检索 → LLM 生成
```
